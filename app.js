...
nav.innerHTML = TABS.map(t => `<button class="tab-btn ${t.id === currentTab ? 'active' : ''}" data-tab="${t.id}" onclick="switchTab('${t.id}')"> ${t.icon}<br>${t.label} </button>`).join(””);
}
...
const pills = TRIP_DAYS.map(d => `<button class="day-pill ${d.id === currentDay ? 'active' : ''}" onclick="selectDay(${d.id})"> Day ${d.id}<span class="pill-date">${d.date}（${d.weekday}）</span> </button>`).join(””);

const timelineItems = day.items.map(item => renderTimelineItem(item)).join(””);

return `${renderWeatherStrip()} <div class="day-pills">${pills}</div> <div class="day-route-card"> <div class="day-route-title">Day ${day.id} · ${day.date}（${day.weekday}）</div> <div class="day-route-flow">${day.route.replace(/➔/g, '<span class="arrow">➔</span>')}</div> </div> <div class="timeline">${timelineItems}</div>`;
}

function renderWeatherStrip() {
const cards = SEOUL_CLIMATE_REFERENCE.map(d => `<div class="weather-card"> <div class="weather-date">${d.label}</div> <div class="weather-icon">${d.icon}</div> <div class="weather-temp">${d.high}°<span class="lo">/${d.low}°</span></div> <div class="weather-desc">${d.desc}</div> <div class="weather-rain">☔️ ${d.rainChance}%</div> </div>`).join(””);
return `<div class="section-title" style="margin-top:0;">🌦️ 首爾天氣（6月下旬氣候參考）</div> <div class="weather-strip">${cards}</div> <div style="font-size:10.5px;color:var(--morandi-brown);margin:-10px 0 18px;line-height:1.5;"> ⓘ 6月底首爾進入梅雨季，數值為歷年同期平均，僅供打包行李參考。出發前建議再查一次即時預報。 </div>`;
}
...
return `<div class="timeline-item ${typeClass}"> <div class="timeline-dot"></div> ${timeTag} <div class="card ${cardClass}"> <div class="card-title ${titleClass}">${icon} ${escapeHtml(item.title)}</div> ${desc} ${link} </div> </div>`;
}
...
return `<div class="checklist-item"> <div class="checklist-checkbox ${checked ? 'checked' : ''}" data-key="${key}" onclick="toggleCheck('${key}')"></div> <div> <div class="checklist-text ${checked ? 'checked-text' : ''}">${escapeHtml(item.text)}</div> ${item.sub ?`<div class="checklist-sub">${escapeHtml(item.sub)}</div>`: ""} ${link} </div> </div>`;
}).join(””);
return `<div class="section-title ${catIdx % 2 === 0 ? '' : 'brown'}">${categoryIcon(cat.category)} ${cat.category}</div> <div class="card" style="margin-bottom:6px;">${itemsHtml}</div>`;
}).join(””);
...
return `<div class="dash-stat" style="margin-bottom:18px;"> <div class="dash-stat-label">準備進度</div> <div class="dash-stat-value">${done} / ${total}</div> <div class="progress-bar-track"><div class="progress-bar-fill" style="width:${total ? (done/total*100) : 0}%"></div></div> </div> ${sections}`;
}
...
return `<div class="checklist-item"> <div class="checklist-checkbox ${checked ? 'checked' : ''}" data-key="${key}" onclick="toggleContentCheck('${key}')"></div> <div class="checklist-text ${checked ? 'checked-text' : ''}">${escapeHtml(item.text)}</div> </div>`;
}).join(””);
...
return `<div class="dash-stat" style="margin-bottom:18px;"> <div class="dash-stat-label">素材拍攝總進度</div> <div class="dash-stat-value">${doneAll} / ${totalAll}</div> <div class="progress-bar-track"><div class="progress-bar-fill" style="width:${totalAll ? (doneAll/totalAll*100) : 0}%"></div></div> </div> ${sections}`;
}
...
return `<div class="section-title red">📍 ${area}</div> ${venues.map(v =>`
